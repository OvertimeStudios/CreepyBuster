...
namespace Prime31
{
	...
	public class GPGMultiplayerManager : AbstractManager
	{
	...
	#if UNITY_IOS
		private delegate void receivedDataCallback( string senderParticipantId, IntPtr dataBuf, int dataSize );

		[DllImport("__Internal")]
		private static extern void _gplaySetReceivedDataCallback( receivedDataCallback callback );


		[AOT.MonoPInvokeCallback( typeof( receivedDataCallback ) )]
		private static void didReceivedData( string senderParticipantId, IntPtr dataBuf, int dataSize )
		{
			var message = new byte[dataSize];
			Marshal.Copy( dataBuf, message, 0, dataSize );
		...
		static GPGMultiplayerManager()
		{
			...
			if( Application.platform == RuntimePlatform.IPhonePlayer )
				_gplaySetReceivedDataCallback( didReceivedData );
	#endif
		}

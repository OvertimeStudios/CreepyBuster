...
namespace Prime31
{
	public class GameCenterManager : AbstractManager
	{
		...
		[Obsolete( "use retrieveScoresForPlayerIdsFailedEvent" )]
		public static event Action<string> retrieveScoresForPlayerIdFailedEvent;

		// Fired when retrieving scores for a playerId fails
		public static event Action<string> retrieveScoresForPlayerIdsFailedEvent;

		[Obsolete( "use scoresForPlayerIdsLoadedEvent" )]
		...
		public static event Action<GameCenterRetrieveScoresResult> scoresForPlayerIdsLoadedEvent;
		...
		{
			if( retrieveScoresForPlayerIdsFailedEvent != null )
				retrieveScoresForPlayerIdsFailedEvent( error );
		}


		void retrieveScoresForPlayerIdDidLoad( string json )
		{
			if( scoresForPlayerIdsLoadedEvent != null )
				scoresForPlayerIdsLoadedEvent( Json.decode<GameCenterRetrieveScoresResult>( json ) );
		}
